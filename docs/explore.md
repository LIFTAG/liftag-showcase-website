...
- `server/api/explore` contains explicit GET handlers. `discoveryApi.ts` validates route/query inputs and provides fixed-path, unauthenticated upstream requests with a 12-second deadline. It forwards no visitor credentials. Private/missing resources return 404; temporary upstream failures return 502. Discovery pages and API responses use `no-store`.
