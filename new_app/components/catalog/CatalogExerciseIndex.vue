...
const muscleFilter = computed(() => {
  if (!isSk) return ''
  return typeof route.query.muscle === 'string' ? route.query.muscle : ''
})
...
const categoryNames = computed(() => {
  ...
  for (const category of categories.value) {
    names.set(category.slug, muscleDisplayName(category.slug, category.name, locale))
  }
  return names
})
...
const filtered = computed<CatalogIndexExercise[]>(() => {
  ...
  if (tokens.length > 0) {
    rows = rows.filter((exercise) => {
      const haystack = isSk
        ? normalizeCatalogQuery(`${exercise.name} ${exercise.aliases ?? ''}`)
        : `${normalizeCatalogQuery(exercise.name)} ${exercise.aliases ?? ''}`
  ...
  <div ref="rootEl" class="ex-index" :class="{ 'is-search-open': searchOpen }">
    <main>
      <header class="ex-hero container">
        <p class="protocol ex-eyebrow">{{ chrome.indexEyebrow }}</p>
        <h1 class="display ex-title">{{ chrome.indexTitleLine1 }}<br><span class="lime">{{ chrome.indexTitleLime }}</span></h1>
        <p class="ex-lead">
          {{ chrome.indexLead }}
        </p>
    ...
    <AppCtaBar :message="chrome.indexCta" />
  </div>
</template>
