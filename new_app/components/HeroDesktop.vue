...
const keepDesktopHeroPhones = computed(() => hasMounted.value && heroFxArmed.value)

const DESKTOP_PARTICLE_COUNT = 1200
const DESKTOP_PARTICLE_DPR_CAP = 1.75

function syncHeroMotionGates(prefersReducedMotion: boolean) {
  const loadDesktop3d = heroFxArmed.value && !prefersReducedMotion
...
function armHeroFx() {
  ...
  syncHeroMotionGates(window.matchMedia('(prefers-reduced-motion: reduce)').matches)
}
...
onMounted(() => {
  ...
  syncHeroMotionGates(prefersReducedMotion)
...
<template>
  <div class="hero-desktop-wrap">
    ...
    <div
      ...
      class="hero-fades hero-desktop-grid"
      :style="{
        position: 'absolute',
        inset: 0,
        pointerEvents: 'none',
