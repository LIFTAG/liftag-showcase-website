...
// 28-div DOM particle layer: ~900 points simulated entirely in the vertex
...
const vertexShader = /* glsl */ `
  ...
  void main() {
    ...
    float twinkle = 0.84 + 0.16 * sin(uTime * (0.6 + aSeed * 1.1) + phase * 3.0);
    float edgeFade = (1.0 - smoothstep(aRangeY * 0.86, aRangeY, abs(y)))
                   * (1.0 - smoothstep(aRangeX * 0.9, aRangeX, abs(x)));

    vAlpha = uReveal * twinkle * mix(0.14, 0.42, depth01) * edgeFade * (1.0 + push * 0.25);
    vTint = aTint;

    vec4 mv = modelViewMatrix * vec4(x, y, p.z, 1.0);
    float size = aSize * uPixelRatio * (1.0 + push * 0.2) * (135.0 / -mv.z);
    gl_PointSize = clamp(size, 1.0, 6.5 * uPixelRatio);
...
const fragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    float a = core * vAlpha;
    if (a < 0.004) discard;
    vec3 col = mix(uColorA, uColorB, vTint) + hot * 0.14;
...
function init() {
  ...
  material = new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      uWall1K: { value: 0 },
      uColorA: { value: new THREE.Color(0.20, 0.22, 0.18) },
      uColorB: { value: new THREE.Color(0.52, 0.66, 0.0) },
    },
  })
