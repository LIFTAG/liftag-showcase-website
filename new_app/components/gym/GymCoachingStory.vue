...
const { frame, ui, selectSource, playRewrite } = useCoachingScroll(root, () => props.reduced);
  ...
  <div ref="story" class="gc-story" :class="{ 'is-reduced': reduced, 'is-owner': ui.isOwner, 'is-gym-video': ui.isGymVideo, 'is-docked': ui.docked }">
    <div class="gc-sticky">
      ...
      <header class="gc-heading">
        <p class="gc-eyebrow"><span class="gc-chapter-number">{{ ui.isOwner ? '04' : '03' }}</span>{{ ui.isOwner ? 'YOUR GYM, ON SCREEN' : 'FROM SCAN TO FIRST REP' }}</p>
        <Transition name="gc-copy" mode="out-in">
          <div :key="ui.isOwner ? 'owner' : 'member'">
            <h2 v-if="ui.isOwner">Your trainer.<br /><em>On every scan.</em></h2>
      ...
      <div class="gc-screen-label" aria-hidden="true"><span class="gc-label-line" /><span>{{ ui.docked ? 'READY ON THE NEXT SCAN' : 'ON YOUR MEMBER’S PHONE' }}</span></div>
      <div class="gc-bottom">
        ...
        <div v-else class="gc-owner-actions">
          <div class="gc-source-switch" role="group" aria-label="Compare instruction videos">
            <button :aria-pressed="!ui.isGymVideo" @click="chooseSource(false)"><span class="gc-source-icon" aria-hidden="true">▷</span>Included guide</button>
            <span class="gc-switch-arrow" aria-hidden="true">→</span>
            <button :aria-pressed="ui.isGymVideo" @click="chooseSource(true)"><span class="gc-source-icon" aria-hidden="true">▷</span>Your gym’s video</button>
          </div>
          <p class="gc-result">{{ ui.isGymVideo ? 'Members scan the same tag. Now they learn from your team.' : 'Every machine starts with a guide from the LIFTAG library.' }}</p>
